...
const Input: React.FunctionComponent<InputProps> = props => {
  ...
  const [inputStyles, setInputStyles] = useState<InputStyles>(getStyles(State.default));
  ...
  if (props.charLimiting > 0) {
    char_limiting = (
      <div style={props.style} className='char-limiting'>
        {props.charLimiting - value.length > 0 ? props.charLimiting - value.length : 0}
      </div>
    );
  }
